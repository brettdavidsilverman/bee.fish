...
movable.onstartmove = function(event) {
    ...
    if (element.classList.contains("movable")) {
        ...
        var startMoveEvent = new Event("startmove");
        startMoveEvent.point = movable.referencePoint;

        // dispatch the event
        if (movable.moving.onstartmove)
            movable.moving.onstartmove(startMoveEvent);

        movable.moving.dispatchEvent(startMoveEvent);
...
movable.onendmove = function(event) {
    ...
    var endMoveEvent = new Event("endmove");
    endMoveEvent.point = movable.referencePoint;

    // dispatch the event
    if (movable.moving.onendmove)
        movable.moving.onendmove(endMoveEvent);

    movable.moving.dispatchEvent(endMoveEvent);
